...
def run_stage_7_resume_tailoring(
    ...
    try:
        ...
        # Save tailored .typ file — use base resume filename for portability
        base_resume_name = os.path.splitext(os.path.basename(config.pipeline.base_resume_path))[0]
...
def run_stage_8_email_generation(
    ...
    try:
        ...
        # Append social links from config.user_identity if not already present
        user_name = config.user_identity.name
